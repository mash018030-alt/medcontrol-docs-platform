...
**Расположение:** репозиторий движка, **`docs-app/engine-docs/OBSHEE-LANDING-SNAPSHOT.md`** (папка регламентов про движок — см. **[README.md](README.md)** в этой же папке). Служебная заметка про интерфейс и код разводящей. **Не путать** с **`docs-app/public/content/references/`** в submodule — там PDF, лого, экспорты руководств и прочий контентный архив.
