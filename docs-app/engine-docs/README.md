...
Внутренние how-to **про код и поведение движка** (не пользовательские статьи MedControl). На сайт и в поиск по статьям **не** попадают. **Не путать** с каталогом **`public/content/`** (пользовательский контент) и путями вида **`0_docs/…`**.
