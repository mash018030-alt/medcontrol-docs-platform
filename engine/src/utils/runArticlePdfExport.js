  ...
  if (!apiUrl) {
    window.alert(
      [
        ...
        '',
        'Локально: npm start в engine/pdf-server; в npm run dev прокси /api/pdf подставляется сам, переменная не обязательна.',
        'Подробнее: engine/pdf-server/README.md и engine/.env.example',
      ].join('\n'),
    )
    return
  }
...
export async function runArticlePdfExport(rootEl, { filename, printUrl = null }) {
    ...
    try {
      ...
      window.alert(
        [
          ...
          '',
          'Проверьте: из каталога engine выполните npm run dev:with-pdf (или в pdf-server: npm install && npm start, порт 3001).',
          ...
          'Инструкция: engine/pdf-server/README.md',
          '',
          msg ? `Детали: ${msg}` : '',
        ]
          .filter(Boolean)
  ...
  if (!apiUrl && !import.meta.env.DEV) {
    console.warn(
      '[PDF] В прод-сборке не задан VITE_PDF_SERVICE_URL — скачивается растровый PDF (html2pdf), не текстовый. См. engine/.env.example и pdf-server/README.md',
    )
  }
  await runArticlePdfExportHtml2Pdf(rootEl, { filename })
}
